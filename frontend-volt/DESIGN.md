...
| 2026-06-08 | Profile is **not** a bottom-nav tab — sidebar only on desktop, and a new 56px mobile top bar (brand + avatar) on `<md`. `navItems` gains a `desktopOnly` flag the bottom nav filters out | A single raised center tab can only stay centered with an **odd** tab count; adding Profile made it 6 and broke the symmetry the elevated "Log" verb depends on. Profile is low-frequency, so it doesn't earn a thumb-reach slot — the avatar is its idiomatic home. One `navItems` source, surface-specific filtering. |
| 2026-06-08 | One account avatar app-wide = Clerk `UserButton` (mobile top bar + desktop sidebar foot), with a custom `UserButton.Action` "Edit profile" routing to `/profile`. Removed the standalone `UserButton`s on Dashboard/Profile and the custom nav-avatar | Two identical circles doing different things (navigate vs. account menu) is a signifier/affordance mismatch. Merging onto one control and disclosing destinations *inside* its menu removes the ambiguity. `Action` + `useNavigate` keeps routing client-side (no `routerPush` configured, so `UserButton.Link` would hard-reload). |
| 2026-07-04 | User-set goals replace the hardcoded `GOALS`/`CALORIE_GOAL` constants in the nutrition views. `GET /goals` 404s until first set, which `useGoals` maps to `null` (loading = `undefined`, set = `Goal`); `resolveGoals(goal)` renames the API's `*Goal` fields to `{calories,protein,carbs,fat}` and falls back to the `shared.ts` defaults while loading/unset | One pure function owns the DTO -> UI rename + defaulting, so the three consumers (`CalorieCard`, `Nutrition`, `NutritionLog`) never branch on load/absence. Defaults become the fallback, not the source of truth. Callers share one query key, so it's a single fetch. |
| 2026-07-04 | Goal setup surfaces as a dismissible Dashboard banner (`GoalsPromptCard`, shown only when `useGoals().data === null`, dismissal in `localStorage`) plus an empty-state Goals card on Profile; both open one create/edit `GoalEntrySheet`. The banner is a plain `/profile` link - no cross-page auto-open | The Profile empty state already signals where to act (Quality Pattern #7), so deep-linking into the sheet via `location.state` added an effect + cascading render for no real gain. Sheet-open stays local state triggered in-page, matching every other page. |
